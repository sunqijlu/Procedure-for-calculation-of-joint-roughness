function [D1,D2] = G_Grasselli_interval_scale_3(joint)
%可用%间距效应-尺寸效应的计算程序
...
for i=1:size(x_left,1)
    if x_left(i,1)==x_right(i,1)
        ...
        x=(1:9:size(joint_rescon,1));
        y=(1:9:size(joint_rescon,1));
        joint_rescon_scale=joint_rescon(x,y,:);
        [C1,C2] = G_Grasselli_anisotropy(joint_rescon_scale);
        D1(:,(i*3-2):i*3)=C1;
        D2(:,i)=C2;
    end
end